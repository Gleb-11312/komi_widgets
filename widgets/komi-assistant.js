  ...
  function mount(selector, opts){
    ...
    <div class="komi-card">
      <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.4rem">
        ...
        <div><b>Komi Assistant</b><div class="komi-muted" style="font-size:.85em">Scripted A1 demo</div></div>
      </div>
    ...
    SCRIPT.forEach(s=>{
      ...
      b.addEventListener('click', async ()=>{ push('user', s.user); setTimeout(async ()=>{ push('bot', s.bot); if(window.KomiTTS) await window.KomiTTS.speak(s.bot); }, 400); });
      $act.appendChild(b);
    });
    push('bot','Салют!'); // short greeting on mount
  }
